...
class GoalEnvNormalizer(object):
    def __init__(self, obs_dims, goal_dims_low, goal_dims_high,
                 input_mean_low, input_var_low,
                 input_mean_high, input_var_high,
        ...
        if input_mean_low is None:
            self.obs_history_mean = np.zeros(self.obs_dims)
            self.goal_history_mean_low = np.zeros(self.goal_dims_low)
        else:
            self.obs_history_mean = input_mean_low[:self.obs_dims]
            self.goal_history_mean_low = input_mean_low[self.obs_dims:]
        if input_var_low is None:
            self.obs_history_var = np.zeros(self.obs_dims)
            self.goal_history_var_low = np.zeros(self.goal_dims_low)
        else:
            self.obs_history_var = input_var_low[:self.obs_dims]
            self.goal_history_var_low = input_var_low[self.obs_dims:]
