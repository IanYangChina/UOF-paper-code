...
class MGPickAndPlaceEnv(multigoal_fetch_env.MultiGoalFetch, utils.EzPickle):
    def __init__(self,
        ...
        multigoal_fetch_env.MultiGoalFetch.__init__(
            ...
            block_size=BLOCK_SIZE, randomized_block_size=randomized_block_size
        )
    ...
    def _renew_goals(self):
        ...
        block_g_pos_on_red = dcp(block_r_pos)
        block_g_pos_on_red[2] += self.block_height

        sub_goals = {
            "pick_blue": np.concatenate([
                # gripper state & position
                self.grasping_gripper_state, (block_b_ground_pos+GRASPING_HEIGHT_OFFSET).ravel(),
            ...
            "blue_on_red": np.concatenate([
                # gripper state & position
                self.grasping_gripper_state, (block_b_pos_on_red+GRASPING_HEIGHT_OFFSET).ravel(),
                # absolute positions of blocks
                block_r_ground_pos.ravel(), block_b_pos_on_red.ravel(), block_g_ground_pos.ravel()
            ]),
            "ending_blue_on_red": np.concatenate([
                # gripper state & position
            ...
            "pick_green": np.concatenate([
                # gripper state & position
                self.grasping_gripper_state, (block_g_ground_pos+GRASPING_HEIGHT_OFFSET).ravel(),
                # absolute positions of blocks
                block_r_ground_pos.ravel(), block_b_ground_pos.ravel(), block_g_ground_pos.ravel()
            ]),
            "green_on_red": np.concatenate([
                # gripper state & position
                self.grasping_gripper_state, (block_g_pos_on_red+GRASPING_HEIGHT_OFFSET).ravel(),
                # absolute positions of blocks
                block_r_ground_pos.ravel(), block_b_ground_pos.ravel(), block_g_pos_on_red.ravel()
            ]),
            "ending_green_on_red": np.concatenate([
                # gripper state & position
                ending_gripper_state, GRIP_END_POS.ravel(),
                # absolute positions of blocks
                block_r_ground_pos.ravel(), block_b_ground_pos.ravel(), block_g_pos_on_red.ravel()
